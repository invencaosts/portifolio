...
test('publica os cases selecionados com slugs únicos', () => {
  assert.equal(projects.length, 4);
  assert.equal(new Set(projects.map((project) => project.slug)).size, projects.length);
  assert.ok(!projects.some((project) => project.slug === 'lacid'));
  for (const project of projects) {
    assert.equal(getProject(project.slug), project);
    assert.ok(project.narrative.length > 0);
...
test('mantém o posicionamento e os vínculos editoriais aprovados', () => {
  ...
  assert.ok(
    academics.some(
      (entry) =>
        entry.id === 'observatorio' &&
        entry.institution.includes('CNPq') &&
        entry.title === 'Observatório Grilagem de Terras Ariovaldo Umbelino de Oliveira',
    ),
  );
  assert.ok(
    academics.some(
      (entry) =>
        entry.id === 'morea' &&
        entry.period.startsWith('Mai/2025') &&
        entry.contributionMode === 'equipe' &&
        entry.description.includes('hardware') &&
        entry.description.includes('plataforma web'),
    ),
  );
  assert.ok(
    academics.some(
      (entry) =>
        entry.id === 'lacid' &&
        entry.period.startsWith('Jan/2026') &&
        entry.kind === 'Estudo independente' &&
        entry.description.includes('cibersegurança'),
    ),
  );
});
