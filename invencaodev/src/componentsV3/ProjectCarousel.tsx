    ...
    <div className="group w-full">
      <div className="mb-6 flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <div className="bg-primary/40 h-1.5 w-1.5 animate-pulse rounded-full"></div>
          <div className="bg-primary/20 h-1.5 w-1.5 rounded-full"></div>
        </div>
      </div>
      ...
      <div className="scrollbar-hide flex snap-x gap-4 overflow-x-auto pb-4">
        {projects.map((project, index) => (
          ...
          >
            {/* Index indicator */}
            <div className="text-foreground/20 group-hover/item:text-primary/40 absolute top-0 right-0 p-3 font-mono text-[8px] transition-colors">
              PRJ_{String(index + 1).padStart(2, '0')}
            </div>

            <div className="mb-4 flex items-start justify-between">
              <FiGithub className="text-primary h-5 w-5" />
              <FiExternalLink className="text-foreground/20 group-hover/item:text-primary h-4 w-4 transition-colors" />
            </div>

            <h3 className="font-headline text-foreground group-hover/item:text-primary mb-2 truncate text-lg font-bold transition-colors">
              {project.name}
            </h3>
